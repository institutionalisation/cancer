...
public class Player {
	...
	public List<Model> colliders = new ArrayList<>();
	...
	public void handleInput(int delta) {
		...
		// collide
		for(Model model : colliders)
		for(Matrix4f transform : model.transforms) {
			model.rootNode.interpolate(transform);
			for(Mesh meshWrapper : model.meshes) {
				//out.println("on mesh "+meshWrapper);
